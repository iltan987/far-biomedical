...
const DOM_ATTRIBUTE = "data-instruments-layout";
...
export function InstrumentsList({ initialLayout }: InstrumentsListProps) {
  ...
  // Sync DOM attribute on layout change (after initial render)
  useEffect(() => {
    document.documentElement.setAttribute(DOM_ATTRIBUTE, layout);
  }, [layout]);

  // Inline script to set DOM attribute immediately (prevents layout shift)
  const initScript = `document.documentElement.setAttribute("${DOM_ATTRIBUTE}","${initialLayout}")`;
  ...
  const setLayout = useCallback((value: LayoutType) => {
    ...
    document.documentElement.setAttribute(DOM_ATTRIBUTE, value);
  }, []);
  ...
  return (
    <>
      {/* Inline script to set DOM attribute before paint (prevents layout shift) */}
      <script
      ...
      <div>
        ...
        {/* Instruments Grid */}
        <div className="instruments-grid" role="list">
